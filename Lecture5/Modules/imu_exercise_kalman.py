...
# other parameters
def plot_accel(imuType, fileName, showPlot = True, show3DLiveView = True, show3DLiveViewInterval = True,
    gyro_bias = {"x" : 0., "y" : 0., "z" : 0.}, gyro_var = 0.001, pitch_var = 0.01, est_var = 2 * pi, gyro_var_acc = 0.001, axis = "pitch"):

    ##### Insert initialize code below ###################

    # approx. bias values determined by averaging over static measurements
    bias_gyro_x = gyro_bias["x"] # [rad/measurement]
    bias_gyro_y = gyro_bias["y"] # [rad/measurement]
    bias_gyro_z = gyro_bias["z"] # [rad/measurement]
    ...
    for line in f:
        ...
        pitch = atan2(acc_y, ( (acc_x ** 2 + acc_z ** 2) ** 0.5))
        ...
        estAngle_hat = None
        if(axis == "pitch"):
            estAngle_hat = estAngle + gyro_x * (ts_now  - ts_prev)
        elif(axis == "roll"):
            estAngle_hat = estAngle + gyro_y * (ts_now  - ts_prev)
        estVar = estVar + gyroVar
        ...
        if(axis == "pitch"):
            ...
            z_k = roll
        K_k = estVar / (estVar + pitchVar)
        estAngle = estAngle_hat + K_k * (z_k - estAngle_hat)
        estVar = estVar * (1. - K_k)
